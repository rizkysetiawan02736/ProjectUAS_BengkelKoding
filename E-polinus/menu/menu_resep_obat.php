    ...
    <div class="card-body">
        <div class="row">
            <div class="form-holder" style="margin-left: 30px;">
                ...
                $query = "SELECT id_periksa FROM periksa WHERE id_periksa='$id_periksa'";
                $result = mysqli_query($koneksi, $query);
                while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) :
                ?>
                    <label class="font-weight-bold">ID Periksa</label>
                    <input style=" width:300px; " autocomplete="off" type="text" name="id_periksa" value="<?= htmlspecialchars($row['id_periksa'], ENT_QUOTES, 'UTF-8'); ?>" required class="form-control" readonly/>
                <?php endwhile; ?>
            </div>
            <br>                                    
            <div class="form-holder" style="margin-left: 30px;">
                ...
                <select id="obat" multiple="multiple" name="obat[]" class="js-example-basic-multiple" style=" width:300px; ">
                    ...
                    if(mysqli_num_rows($result) > 0){
                        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                            ?>
                            <option value="<?= htmlspecialchars($row['id'], ENT_QUOTES, 'UTF-8'); ?>"><?= htmlspecialchars($row['nama_obat'], ENT_QUOTES, 'UTF-8'); ?> </option>
                            <?php
                        }
                    } else {
                        ?> <option value="No record found">No record found</option><?php
                    }
                    ?>
                </select>
            </div>
